...
int NACA1512_InitialSolution(ParametersType *Parameters, NodeType *Node, double *u)
{
	...
	for(I = 0; I < nnodes; I++)
	{
		...
		if (Node[I].id[1] >= 0){ 
			//if (Node[I].v1Type<0){
			//	theta = NACA1512_theta(Node[I].x, Node[I].y);
			//	u[Node[I].id[1]] = cos(theta); //ajustando a condição inicial para a malha do naca			
			//}
			//else	
				u[Node[I].id[1]] = cos(0.261799387799149436538);
		}
		if (Node[I].id[2] >= 0){ 
			u[Node[I].id[2]] = sin(0.261799387799149436538); 
		}
		if (Node[I].id[3] >= 0){ 
			u[Node[I].id[3]] = NACA1512_epresc(Parameters,0,0); // we used x=0 and y=0 because x and y are not necessary
		}	
