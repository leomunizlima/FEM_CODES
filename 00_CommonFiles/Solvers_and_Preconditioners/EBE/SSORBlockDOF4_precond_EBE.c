 ...
 * z out
*/
int SSORBlockDOF4_precond_EBE (ParametersType *Parameters, MatrixDataType *MatrixData,
	               double *p, double *z) {
	...
	int **lm = MatrixData->lm; //array LM
	double **A = MatrixData->A[0]; //matrix data
	double **inv = MatrixData->invDe; //1 / (1 + D)

	double omega;
	omega = atof(&(Parameters->Preconditioner[9])); // In SORw, w is the weight coefficient
