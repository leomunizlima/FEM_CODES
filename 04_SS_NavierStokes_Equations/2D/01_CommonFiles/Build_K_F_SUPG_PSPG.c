...
/*(ParametersType *Parameters, MatrixDataType *MatrixData, NodeType *Node, ElementType *Element, double *F, double *u, int **lm,
...
*/	
{
	...
	double C1, C2, C3;
	double Nv[6], Ndeltav[6], Kv[6], Gv[6], Gdeltav[6], GTv[3], Nphiv[3], Gphiv[3], Res[9];
...
/*		fy3 = (*Font)(X[2],Y[2],2);*/
 ...
 4*pow((-1 + x),2)*pow(x,2)*(1 - 6*x + 6*pow(x,2))*pow((-1 + y),2)*pow(y,3)*(1 - 3*y + 2*pow(y,2)) + pow(x,3)*(0.8 - 4.8*y + 4.8*pow(y,2)) + x*(0.4 - 2.4*y + 4.8*pow(y,2) - 4.8*pow(y,3) + 2.4*pow(y,4));
		...
		Res[0] = Fe[0] - (Nv[0] + Ndeltav[0] + Kv[0] - ( Gv[0] + Gdeltav[0] ) );
		Res[1] = Fe[1] - (Nv[1] + Ndeltav[1] + Kv[1] - ( Gv[1] + Gdeltav[1] ) );
		Res[2] = Fe[2] - ( GTv[0] + Nphiv[0] + Gphiv[0] );
		Res[3] = Fe[3] - (Nv[2] + Ndeltav[2] + Kv[2] - ( Gv[2] + Gdeltav[2] ) );
		Res[4] = Fe[4] - (Nv[3] + Ndeltav[3] + Kv[3] - ( Gv[3] + Gdeltav[3] ) );
		Res[5] = Fe[5] - ( GTv[1] + Nphiv[1] + Gphiv[1] );
		Res[6] = Fe[6] - (Nv[4] + Ndeltav[4] + Kv[4] - ( Gv[4] + Gdeltav[4] ) );
		Res[7] = Fe[7] - (Nv[5] + Ndeltav[5] + Kv[5] - ( Gv[5] + Gdeltav[5] ) );
		Res[8] = Fe[8] - ( GTv[2] + Nphiv[2] + Gphiv[2] );
				
		//**********************Tangent matrix*******************************
		//op = 1; // 0 se ISI e 1 se NI
		//if(op==0){		
		//if(varglobal < 6){ //varglobal (<6 ISI, Ponto Fixo) (>=6 NI. Met. de Newton)
