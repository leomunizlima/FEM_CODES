...
/*
int Build_K_F_SUPG_PSPG(int neq, int nedge, int nnzero, int nel, MatrixDataType *MatrixData, double *F, double *u, NodeType *Node, ElementType *Element,
void (*Assembly)(int, int, int, double [9][9], MatrixDataType *), double (*ppresc)(double, double), double (*v1presc)(double, double), double (*v2presc)(double, double))
*/
...
//, double (*Viscosity)(),double (*Rho)() , double (*U_ref)(), double (*L_ref)())	
{
	...
	//double Uref, Lref;
	double TwoA, invArea, Area, tau, h, visc, rho, invrho, nu, unorm, auxtau, soma; 
	...
	vet_bound = (double*) mycalloc("vet_bound of 'Build_K_F_SUPG_PSPG'", NDOF*NNOEL, sizeof(double));
...
/*		fy3 = (*Font)(X[2],Y[2],2);*/
		...
		Ndeltav[0] = rho*tau*C1*( duxdx*ux + duxdy*uy );
		Ndeltav[1] = rho*tau*C1*( duydx*ux + duydy*uy );
		Ndeltav[2] = rho*tau*C2*( duxdx*ux + duxdy*uy );
		Ndeltav[3] = rho*tau*C2*( duydx*ux + duydy*uy );
		Ndeltav[4] = rho*tau*C3*( duxdx*ux + duxdy*uy );
		Ndeltav[5] = rho*tau*C3*( duydx*ux + duydy*uy );
		...
		Gv[0] = 0.5*p*invArea*y23; 
		Gv[1] = 0.5*p*invArea*x32;
		Gv[2] = 0.5*p*invArea*y31;
		Gv[3] = 0.5*p*invArea*x13;
		Gv[4] = 0.5*p*invArea*y12;
		Gv[5] = 0.5*p*invArea*x21;

		Gdeltav[0] = tau*C1*dpdx;
		Gdeltav[1] = tau*C1*dpdy;
		Gdeltav[2] = tau*C2*dpdx;
		Gdeltav[3] = tau*C2*dpdy;
		Gdeltav[4] = tau*C3*dpdx;
		Gdeltav[5] = tau*C3*dpdy;
		
		//Residue R
		RQres[0] = Fe[0] - (Nv[0] + Ndeltav[0] + Kv[0] - ( Gv[0] + Gdeltav[0] ) );
		RQres[1] = Fe[1] - (Nv[1] + Ndeltav[1] + Kv[1] - ( Gv[1] + Gdeltav[1] ) );
		RQres[2] = Fe[2] - (Nv[2] + Ndeltav[2] + Kv[2] - ( Gv[2] + Gdeltav[2] ) );
		RQres[3] = Fe[3] - (Nv[3] + Ndeltav[3] + Kv[3] - ( Gv[3] + Gdeltav[3] ) );
		RQres[4] = Fe[4] - (Nv[4] + Ndeltav[4] + Kv[4] - ( Gv[4] + Gdeltav[4] ) );
		RQres[5] = Fe[5] - (Nv[5] + Ndeltav[5] + Kv[5] - ( Gv[5] + Gdeltav[5] ) );
		...
		Gphiv[0] = 0.5*tau*invArea*invrho*( y23*dpdx + x32*dpdy);
		Gphiv[1] = 0.5*tau*invArea*invrho*( y31*dpdx + x13*dpdy);
		Gphiv[2] = 0.5*tau*invArea*invrho*( y12*dpdx + x21*dpdy);

		//Residue Q
		RQres[6] = Fe[6] - ( GTv[0] + Nphiv[0] + Gphiv[0] );
		RQres[7] = Fe[7] - ( GTv[1] + Nphiv[1] + Gphiv[1] );
		...
		Ke[0][2] = N13 + Np31 + Ndelta13 + Npdelta11 + Nppdelta11 + K13;
		Ke[0][3] =		 Np32 	 		 + Npdelta12 + Nppdelta12 + K14;
		Ke[0][4] = N15 + Np31 + Ndelta15 + Npdelta11 + Nppdelta11 + K15;
		Ke[0][5] = 		 Np32 	 		 + Npdelta12 + Nppdelta12 + K16;
		Ke[0][6] = -( GT11 + Gdelta11 );
		Ke[0][7] = -( GT11 + Gdelta12 );
		Ke[0][8] = -( GT11 + Gdelta13 );
	
		Ke[1][0] = 		 Np21            + Npdelta21 + Nppdelta21 + K12;
		Ke[1][1] = N11 + Np22 + Ndelta22 + Npdelta22 + Nppdelta22 + K22;
		Ke[1][2] = 		 Np41			 + Npdelta21 + Nppdelta21 + K23;
		Ke[1][3] = N13 + Np42 + Ndelta24 + Npdelta22 + Nppdelta22 + K24;
		Ke[1][4] = 		 Np41 			 + Npdelta21 + Nppdelta21 + K25;
		Ke[1][5] = N15 + Np42 + Ndelta26 + Npdelta22 + Nppdelta22 + K26;
		Ke[1][6] = -( GT12 + Gdelta21 );
		Ke[1][7] = -( GT12 + Gdelta22 );
		Ke[1][8] = -( GT12 + Gdelta23 );
		
		Ke[2][0] = N11 + Np31 + Ndelta13 + Npdelta31 + Nppdelta31 + K13;
		Ke[2][1] =       Np32 			 + Npdelta32 + Nppdelta32 + K23;
		Ke[2][2] = N13 + Np11 + Ndelta33 + Npdelta31 + Nppdelta31 + K33;
		Ke[2][3] =		 Np12 	 		 + Npdelta32 + Nppdelta32 + K34;
		Ke[2][4] = N15 + Np31 + Ndelta35 + Npdelta31 + Nppdelta31 + K35;
		Ke[2][5] = 		 Np32 	 		 + Npdelta32 + Nppdelta32 + K36;
		Ke[2][6] = -( GT13 + Gdelta31 );
		Ke[2][7] = -( GT13 + Gdelta32 );
		Ke[2][8] = -( GT13 + Gdelta33 );

		Ke[3][0] = 		 Np41            + Npdelta41 + Nppdelta41 + K14;
		Ke[3][1] = N11 + Np42 + Ndelta24 + Npdelta42 + Nppdelta42 + K24;
		Ke[3][2] = 		 Np21			 + Npdelta41 + Nppdelta41 + K34;
		Ke[3][3] = N13 + Np22 + Ndelta44 + Npdelta42 + Nppdelta42 + K44;
		Ke[3][4] = 		 Np41 			 + Npdelta41 + Nppdelta41 + K45;
		Ke[3][5] = N15 + Np42 + Ndelta46 + Npdelta42 + Nppdelta42 + K46;
		Ke[3][6] = -( GT14 + Gdelta41 );
		Ke[3][7] = -( GT14 + Gdelta42 );
		Ke[3][8] = -( GT14 + Gdelta43 );

		Ke[4][0] = N11 + Np31 + Ndelta15 + Npdelta51 + Nppdelta51 + K15;
		Ke[4][1] =       Np32 			 + Npdelta52 + Nppdelta52 + K25;
		Ke[4][2] = N13 + Np31 + Ndelta35 + Npdelta51 + Nppdelta51 + K35;
		Ke[4][3] =		 Np32 	 		 + Npdelta52 + Nppdelta52 + K45;
		Ke[4][4] = N15 + Np11 + Ndelta55 + Npdelta51 + Nppdelta51 + K55;
		Ke[4][5] = 		 Np12 	 		 + Npdelta52 + Nppdelta52 + K56;
		Ke[4][6] = -( GT15 + Gdelta51 );
		Ke[4][7] = -( GT15 + Gdelta52 );
		Ke[4][8] = -( GT15 + Gdelta53 );

		Ke[5][0] = 		 Np41            + Npdelta61 + Nppdelta61 + K16;
		Ke[5][1] = N11 + Np42 + Ndelta26 + Npdelta62 + Nppdelta62 + K26;
		Ke[5][2] = 		 Np41			 + Npdelta61 + Nppdelta61 + K36;
		Ke[5][3] = N13 + Np42 + Ndelta46 + Npdelta62 + Nppdelta62 + K46;
		Ke[5][4] = 		 Np21 			 + Npdelta61 + Nppdelta61 + K56;
		Ke[5][5] = N15 + Np22 + Ndelta66 + Npdelta62 + Nppdelta62 + K66;
		Ke[5][6] = -( GT16 + Gdelta61 );
		Ke[5][7] = -( GT16 + Gdelta62 );
		Ke[5][8] = -( GT16 + Gdelta63 );

		Ke[6][0] = GT11 + Gdelta11 + Npphi11;
		Ke[6][1] = GT12 + Gdelta21 + Npphi12;
		Ke[6][2] = GT13 + Gdelta31 + Npphi11;
		Ke[6][3] = GT14 + Gdelta41 + Npphi12;
		Ke[6][4] = GT15 + Gdelta51 + Npphi11;
		Ke[6][5] = GT16 + Gdelta61 + Npphi12;
		Ke[6][6] = Gphi11;
		Ke[6][7] = Gphi12;
		Ke[6][8] = Gphi13;

		Ke[7][0] = GT11 + Gdelta12 + Npphi21;
		Ke[7][1] = GT12 + Gdelta22 + Npphi22;
		Ke[7][2] = GT13 + Gdelta32 + Npphi21;
		Ke[7][3] = GT14 + Gdelta42 + Npphi22;
		Ke[7][4] = GT15 + Gdelta52 + Npphi21;
		Ke[7][5] = GT16 + Gdelta62 + Npphi22;
		Ke[7][6] = Gphi12;
		Ke[7][7] = Gphi22;
		Ke[7][8] = Gphi23;
		...
		Ke[8][2] = GT13 + Gdelta33 + Npphi31;
		Ke[8][3] = GT14 + Gdelta43 + Npphi32;
		Ke[8][4] = GT15 + Gdelta53 + Npphi31;
		Ke[8][5] = GT16 + Gdelta63 + Npphi32;
		Ke[8][6] = Gphi13;
		Ke[8][7] = Gphi23;
		Ke[8][8] = Gphi33;

		// Vetor Fe local do Elemento
		//velocidades v1 e v2 nos nos 1, 2 e 3		
		...
		if (Node[J1].id[2]==-1){ // nó prescrito
			vet_bound[2] = v1presc(X[1],Y[1]);
		}else{ // nó incógnita
			vet_bound[2] = 0.0;
		} 
		if (Node[J1].id[3]==-1){ // nó prescrito
			vet_bound[3] = v2presc(X[1],Y[1]);
		}else{ // nó incógnita
			vet_bound[3] = 0.0;
		} 
		if (Node[J1].id[4]==-1){ // nó prescrito
			vet_bound[4] = v1presc(X[2],Y[2]);
		}else{ // nó incógnita
			vet_bound[4] = 0.0;
		} 
		if (Node[J1].id[5]==-1){ // nó prescrito
			vet_bound[5] = v2presc(X[2],Y[2]);
		}else{ // nó incógnita
			vet_bound[5] = 0.0;
		} 
		//presao p nos nos 1, 2 e 3		
		/*if (Node[J1].id[6]==-1){ // nó prescrito
			vet_bound[6] = ppresc(X[0],Y[0]);
		}else{ // nó incógnita
			vet_bound[6] = 0.0;
			} 
		if (Node[J1].id[7]==-1){ // nó prescrito
			vet_bound[7] = ppresc(X[1],Y[1]);
		}else{ // nó incógnita
			vet_bound[7] = 0.0;
		}
		if (Node[J1].id[8]==-1){ // nó prescrito
